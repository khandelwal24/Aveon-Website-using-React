      ...
      <div className={`fixed inset-0 bg-gray-800 bg-opacity-50 transition-opacity ${Nav ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setNav(!Nav)}></div>
        ...
        <div className="flex justify-between items-center py-3">
            ...
            <div className="text-2xl space-x-5 lg:block hidden" style={{fontFamily:'Orbitron'}}>
                <NavLink to ={"/#Home"} className="hover:text-gray-500 hover:border-b-[2px] border-white hover:transition-all ease-in-out duration-75 ">Home</NavLink>
                <NavLink to ={"/#About"} className="hover:text-gray-500 hover:border-b-[2px] border-white hover:transition-all ease-in-out duration-75 ">About</NavLink>
                ...
                <NavLink to ={"/#Sponsor"} className="hover:text-gray-500 hover:border-b-[2px] border-white hover:transition-all ease-in-out duration-75">Sponsors</NavLink>
...
<button onClick={()=>setNav(!Nav)} className='mmd:hidden'>
      {!Nav ? <GiHamburgerMenu  className='text-3xl'/> : <IoClose className='text-3xl text-white' />}
        ...
        <img src={logo} alt='logo' width={200} className= {`text-3xl font-bold text-white mb-6 px-4 py-[14px] ${ !Nav ? 'hidden' : 'block'} `}/>
          <NavLink to={"/#Home"} className=" border-b-[1px] rounded-xl border-white block py-3 px-2 mb-3 hover:bg-[#213068] hover:transition-all ease-in-out duration-75">Home</NavLink>
          <NavLink to={"/#About"} className=" border-b-[1px] rounded-xl border-white block py-3 px-2 mb-3 hover:bg-[#213068] hover:transition-all ease-in-out duration-75">About</NavLink>
          ...
          <NavLink to={"/#Sponsor"} className=" border-b-[1px] rounded-xl border-white block py-3 px-2 mb-3 hover:bg-[#213068] hover:transition-all ease-in-out duration-75">Sponsors</NavLink>
          ...
          <a href={pdf} download="Team Aveon Broucher.pdf" className={`p-2.5 bg-gray-600 text-white rounded-full mt-7 border-2 border-white ${(Nav) ? 'block' : 'hidden'} hover:bg-gray-500`}>Download Broucher <i className="fa-solid fa-download"></i> </a>
      </div>
    </nav>
